...
def reset_katy():
    """Reseta todas as variáveis globais da fase Katy para reiniciar"""
...
def achar_y_em_cima_do_bloco(x_pos, largura_inimigo=70, altura_inimigo=70):
    ...
    for bloco in blocks:
        if bloco.rect.left <= centro_x <= bloco.rect.right:
            candidates.append(bloco)

    if candidates:
        bloco_mais_alto = min(candidates, key=lambda b: b.rect.top)
...
def katy_tela(window):
    ...
    if not musica_tocando:
        ...
        pygame.mixer.music.load('assests/sons/california.ogg')
        pygame.mixer.music.play(-1)
        musica_tocando = True

    # carrega a imagem do sprite da Katy com a pasta 'assests'
    if sprite is None:
        sprite = pygame.image.load('assests/sprites/katy_sprite.png').convert_alpha()
    ...
    # desenha fundo
    background = pygame.image.load('assests/imagens/tela katy perry.png').convert()
    ...
    # CRITÉRIO DE VITÓRIA: Se pegar 10 moedas, passa para a Lady Gaga!
    if pontos >= 10:
        return SCREEN_LADYGAGA
