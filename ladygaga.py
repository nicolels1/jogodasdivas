...
def reset_ladygaga():
    """Reseta todas as variáveis globais da fase Lady Gaga para reiniciar"""
...
def ladygaga_tela(window):
    ...
    if not musica_tocando:
        ...
        pygame.mixer.music.load('assests/sons/ladygaga.ogg')
        pygame.mixer.music.play(-1)
        musica_tocando = True

    # carrega a imagem só na primeira vez
    if sprite is None:
        sprite = pygame.image.load('assests/sprites/ladygaga_sprite.png').convert_alpha()
    ...
    # CRITÉRIO DE VITÓRIA: Se pegar 10 moedas, passa para a vitória!
    if pontos >= 10:
        return SCREEN_WIN
